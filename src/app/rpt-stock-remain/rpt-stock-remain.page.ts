...
@Component({
  selector: 'app-rpt-stock-remain',
  templateUrl: './rpt-stock-remain.page.html',
  styleUrls: ['./rpt-stock-remain.page.scss'],
})
export class RptStockRemainPage implements OnInit {
  public resRptStockRemain: any;
  ...
  public exportPDF() {
    ...
    doc.html(dataHTML, {
      callback: function (doc) {
        doc.save('รายงาน - Stock คงเหลือ.pdf');
      },
      autoPaging: true,
      margin: [20, 0, 20, 0],
      x: 0,
  ...
  async getRPT() {
    ...
    await this.storage.get('appProfile').then((value) => {
      ...
      if (value != null) {
        this.http.get(this.myAppComp.strAPIURL + '/getRptStockRemain',this.myAppComp.getHttpHeader(value))
          .subscribe(data => {
            //console.log(data);
            ...
            this.resRptStockRemain = data['data'];
            if (this.resRptStockRemain.length > 0) {
